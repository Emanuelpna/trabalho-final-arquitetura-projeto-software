...
public class DrugstoreOrder extends Order {
    public DrugstoreOrder(UUID id, Address shippingAddress, List<OrderItem> orderItems, OrderState state) {
        super(id, shippingAddress, orderItems, state);
    }

    public DrugstoreOrder(Address shippingAddress, List<OrderItem> orderItems, OrderState state) {
    ...
    public void processOrder() {
        for (OrderItem orderItem : getOrderItems()) {
            if (!orderItem.getProduct().getStoreType().equals(StoreType.DRUGSTORE)) {
